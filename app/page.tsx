    ...
    <main className="admin-shell">
      ...
      <section className="dashboard-main">
        ...
        <section className="dash-grid">
          ...
          <article className="dash-panel map-panel"><PanelTitle n="3" title="ANALISIS HOTSPOT – ADUAN SPATIAL"/><div className="heat-map real-kl-map"><iframe key={hotspotData[hotspot].name} className="real-map-frame" title={`Peta ${hotspotData[hotspot].name}, Kuala Lumpur`} loading="lazy" src={`https://www.openstreetmap.org/export/embed.html?bbox=101.665%2C3.110%2C101.730%2C3.180&layer=mapnik&marker=${hotspotData[hotspot].lat}%2C${hotspotData[hotspot].lon}`}/><div className="map-dim"/>{hotspotData.map((x,i)=><button key={x.name} className={`map-marker ${hotspot===i?"active":""} ${x.risk==="Tinggi"?"high":"medium"}`} style={{left:x.x,top:x.y}} onClick={()=>setHotspot(i)} aria-label={`${x.name}, ${x.cases} aduan, risiko ${x.risk}`}><i>{i+1}</i><span>{x.name}</span></button>)}<div className="map-focus"><small>LOKASI DIPILIH · {hotspotData[hotspot].lat.toFixed(4)}, {hotspotData[hotspot].lon.toFixed(4)}</small><strong>{hotspotData[hotspot].name} · {hotspotData[hotspot].cases} aduan</strong><span>{hotspotData[hotspot].detail}</span><a href={`https://www.openstreetmap.org/?mlat=${hotspotData[hotspot].lat}&mlon=${hotspotData[hotspot].lon}#map=16/${hotspotData[hotspot].lat}/${hotspotData[hotspot].lon}`} target="_blank" rel="noreferrer">Buka peta penuh ↗</a></div><a className="map-attribution" href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">© OpenStreetMap contributors</a></div><ol>{hotspotData.map((x,i)=><li key={x.name} className={hotspot===i?"active":""}><button onClick={()=>setHotspot(i)}><span><b>{i+1}. {x.name}</b><small>{x.cases} aduan · {x.lat.toFixed(4)}, {x.lon.toFixed(4)}</small></span><em>{x.risk}</em></button></li>)}</ol></article>
...
function DemoLogin({lang,onLogin}:{lang:Lang;onLogin:()=>void}){
  ...
  return <main className="login-page"><section className="login-brand-panel"><div className="login-official"><img src="/logo-jim.png" alt="Logo rasmi Jabatan Imigresen Malaysia"/><div><b>JABATAN IMIGRESEN MALAYSIA</b><span>WILAYAH PERSEKUTUAN KUALA LUMPUR</span></div></div><div className="login-intro"><span>{tr(lang,"SISTEM ADUAN PINTAR","SMART COMPLAINT SYSTEM")}</span><h1>{tr(lang,"Pusat Kawalan Operasi Bersepadu","Integrated Operations Control Centre")}</h1><p>{tr(lang,"Pantau aduan, analisis risiko dan tindakan operasi melalui satu paparan yang selamat dan responsif.","Monitor complaints, risk analysis and operational actions through one secure, responsive interface.")}</p><div className="login-features"><span><i>✓</i>{tr(lang,"Data simulasi yang realistik","Realistic simulated data")}</span><span><i>✓</i>{tr(lang,"Dashboard operasi interaktif","Interactive operations dashboard")}</span><span><i>✓</i>{tr(lang,"Akses pelanggan tanpa akaun sebenar","Customer access without a real account")}</span></div></div><small className="login-classification">DEMO PELANGGAN · BUKAN SISTEM PRODUKSI</small></section><section className="login-form-panel"><div className="login-card"><div className="login-card-head"><span>{tr(lang,"AKSES DEMONSTRASI","DEMONSTRATION ACCESS")}</span><h2>{tr(lang,"Log masuk ke Sistem Aduan Pintar","Sign in to Smart Complaint System")}</h2><p>{tr(lang,"Gunakan akaun demo atau teruskan sebagai pelawat.","Use the demo account or continue as a guest.")}</p></div><form onSubmit={submit}><label><span>{tr(lang,"Alamat e-mel","Email address")}</span><div className="login-input"><i>✉</i><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="nama@imigresen.gov.my" autoComplete="username" required/></div></label><label><span>{tr(lang,"Kata laluan","Password")}</span><div className="login-input"><i>●</i><input type={showPassword?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} placeholder="Masukkan kata laluan" autoComplete="current-password" required/><button type="button" onClick={()=>setShowPassword(v=>!v)} aria-label={tr(lang,"Tunjuk atau sembunyikan kata laluan","Show or hide password")}>{showPassword?"Sembunyi":"Lihat"}</button></div></label>{error&&<div className="login-error" role="alert">⚠ {error}</div>}<button className="login-submit" type="submit">{tr(lang,"Log Masuk Demo","Demo Sign In")} →</button></form><div className="demo-credentials"><div><span>{tr(lang,"AKAUN CONTOH","SAMPLE ACCOUNT")}</span><button onClick={fillDemo}>{tr(lang,"Isi Automatik","Auto-fill")}</button></div><p><b>E-mel</b><code>{demoEmail}</code></p><p><b>{tr(lang,"Kata laluan","Password")}</b><code>{demoPassword}</code></p></div><div className="login-separator"><span>{tr(lang,"atau","or")}</span></div><button className="guest-login" onClick={onLogin}>{tr(lang,"Teruskan sebagai Pelawat","Continue as Guest")}</button><p className="login-disclaimer">ⓘ {tr(lang,"Ini ialah halaman log masuk dummy untuk tujuan demonstrasi. Tiada data peribadi atau pengesahan identiti sebenar digunakan.","This is a dummy login page for demonstration. No personal data or real identity verification is used.")}</p></div></section></main>
}
...
export default function Home(){
  ...
  useEffect(()=>{if(window.sessionStorage.getItem("jim-demo-session")==="active")setAuthenticated(true)},[]);
  useEffect(()=>{window.localStorage.setItem("jim-display-preferences",JSON.stringify({lang,theme,fontSize}))},[lang,theme,fontSize]);
  const login=()=>{window.sessionStorage.setItem("jim-demo-session","active");setAuthenticated(true)}; const logout=()=>{window.sessionStorage.removeItem("jim-demo-session");setView("dashboard");setAuthenticated(false)};
