...
function Dashboard({ setView, lang, role, onLogout }: { setView: (v: View) => void; lang: Lang; role: AdminRole; onLogout: () => void }) {
  ...
  const periodData={
    "Hari Ini":{received:"1,050",process:"350",done:"700",critical:"50",response:"2.1 jam",ops:"24",arrests:"156",newCases:"12",score:"66.6%",sources:[["SISPAA","483","46%"],["E-MEL","252","24%"],["SURAT","105","10%"],["HADIR (WALK IN)","126","12%"],["TELEFON","84","8%"]],categories:{high:"50",medium:"430",low:"570"}},
    "7 Hari":{received:"7,842",process:"1,306",done:"6,478",critical:"214",response:"2.4 jam",ops:"137",arrests:"684",newCases:"68",score:"90%",sources:[["SISPAA","3,606","46%"],["E-MEL","1,882","24%"],["SURAT","784","10%"],["HADIR (WALK IN)","942","12%"],["TELEFON","628","8%"]],categories:{high:"214",medium:"3,156",low:"4,472"}},
    "30 Hari":{received:"31,608",process:"3,914",done:"27,122",critical:"906",response:"2.8 jam",ops:"528",arrests:"2,431",newCases:"286",score:"88%",sources:[["SISPAA","14,539","46%"],["E-MEL","7,586","24%"],["SURAT","3,161","10%"],["HADIR (WALK IN)","3,793","12%"],["TELEFON","2,529","8%"]],categories:{high:"906",medium:"12,844",low:"17,858"}},
  }[period];
  const hotspotData=[
      ...
      <section className="dashboard-main">
        ...
        {activeMenu==="AI Pengkelasan"?<ClassificationDemo lang={lang}/>:<><div className="activity-ribbon"><span><i className="pulse"/> {tr(lang,"Sistem menerima","System received")} <b>{periodData.newCases} {tr(lang,"aduan baharu","new complaints")}</b> · {period}</span><span>✦ AI {tr(lang,"mengklasifikasi","classified")} <b>98.7%</b> {tr(lang,"tanpa semakan manual","without manual review")}</span><span>⌖ {tr(lang,"Hotspot aktif","Active hotspot")}: <b>{hotspotData[hotspot].name}</b></span></div><section className={`stats-row period-updated ${["Dashboard","Operasi","Hasil Operasi"].includes(activeMenu)?"":"module-hidden"}`} key={period}>
          ...
          <article className="stat-card score">
            <div className="mini-ring" style={{background:`radial-gradient(circle,#041d36 54%,transparent 56%),conic-gradient(#19e69b ${periodData.score},#153b4c 0)`}} aria-label={`${tr(lang,"Kadar prestasi","Performance rate")} ${periodData.score}`}><b>{periodData.score}</b></div>
            <div className="score-copy"><small>{tr(lang,"PRESTASI OPERASI","OPERATION PERFORMANCE")}</small><strong className="score-status"><i/> {tr(lang,"Sangat Baik","Excellent")}</strong><em>{periodData.done} {tr(lang,`daripada ${periodData.received} aduan selesai`,`of ${periodData.received} complaints completed`)}</em></div>
          </article>
        </section>
...
function ClassificationDemo({lang}:{lang:Lang}){
  ...
  return <section className="classification-view">
    ...
    <div className="source-strip"><b>SUMBER DATA ADUAN (MULTI-SOURCE):</b>{[["SISPAA",483,"46%"],["E-mel",252,"24%"],["Surat",105,"10%"],["Walk-in",126,"12%"],["Telefon",84,"8%"]].map(([n,c,p],i)=><span key={String(n)}><i className={`src-dot s${i}`}/>{n} <em>{c} · {p}</em></span>)}</div>
