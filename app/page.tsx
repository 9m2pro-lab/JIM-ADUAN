...
function PortalHeader({ view, setView, lang, role, onLogout }: { view: View; setView: (v: View) => void; lang: Lang; role: UserRole; onLogout: () => void }) {
  ...
  const allowedViews:View[]=role==="complaints"?["aduan","semakan","maklumat","bantuan"]:["dashboard"];
  const homeView:View=role==="complaints"?"aduan":"dashboard";
  return (
...
function Dashboard({ setView, lang, role, onLogout }: { setView: (v: View) => void; lang: Lang; role: UserRole; onLogout: () => void }) {
  ...
  const allSide = [["▦","Dashboard"],["◇","Aduan"],["⌘","Operasi"],["▥","AI Pengkelasan"],["✦","AI Cadangan"],["◎","AI Pengesahan"],["⚑","Hasil Operasi"],["⌖","Peta Hotspot"],["▤","Laporan"],["♢","Notifikasi"],["⚙","Tetapan"]];
  const side=role==="management"?allSide.filter(([,label])=>label==="Dashboard"):allSide.filter(([,label])=>label!=="Dashboard"&&label!=="Aduan");
...
export default function Home(){
  ...
  const setAuthorizedView=(next:View)=>{if(role==="complaints"){setView((["aduan","semakan","maklumat","bantuan","berjaya"] as View[]).includes(next)?next:"aduan")}else setView("dashboard")};
  const portal=role==="complaints"; const content=useMemo(()=>{if(role==="complaints"){if(view==="semakan")return <Tracking/>;if(view==="maklumat")return <InfoPage/>;if(view==="bantuan")return <InfoPage help/>;if(view==="berjaya")return <Success setView={setAuthorizedView}/>;return <ComplaintForm setView={setAuthorizedView}/>}if(role)return <Dashboard setView={setAuthorizedView} lang={lang} role={role} onLogout={logout}/>;return null},[view,lang,role]);
  return <div className={`experience-root theme-${theme} font-${fontSize}`} lang={lang==="BM"?"ms":"en"}><AccessibilityToolbar lang={lang} setLang={setLang} theme={theme} setTheme={setTheme} fontSize={fontSize} setFontSize={setFontSize}/><div className="app-surface">{!role?<DemoLogin lang={lang} onLogin={login}/>:<>{portal&&<PortalHeader view={view} setView={setAuthorizedView} lang={lang} role={role} onLogout={logout}/>} {content}{portal&&<footer><span>{tr(lang,"© 2025 Jabatan Imigresen Malaysia. Hak Cipta Terpelihara.","© 2025 Immigration Department of Malaysia. All Rights Reserved.")}</span><span>{tr(lang,"Dasar Privasi　|　Terma Penggunaan　|　Panduan Pengguna","Privacy Policy　|　Terms of Use　|　User Guide")}</span><span>{roleName(role,lang)} · {tr(lang,"Demo pengalaman pelanggan","Customer experience demo")}</span></footer>}</>}</div></div>
}
